...
extension UIViewController {
    ...
    public func getBaseRequestData<T:Mappable> (data:T)-> BaseApiRequestBody<T>{
        ...
        return BaseApiRequestBody.init(token:  "", languageId: userDefaultsData.getLanguageId(), mobilInformation: "\(userDefaultsData.getDeviceId() ?? "");\(deviceModel);iOS;\(systemVersion); Wifi; \(appVersion)",data: data)
    }
    ...
    public func getBaseRequestArrayData<T:Mappable> (dataArray:[T])-> BaseApiRequestBody<T>{
        ...
        let deviceModel = UIDevice.current.modelName
        let appVersion = Bundle.main.infoDictionary?["CFBundleShortVersionString"] as? String ?? ""
        let baseApiRequestBodyModel = BaseApiRequestBody.init(token: "", languageId: userDefaultsData.getLanguageId(), mobilInformation: "\(deviceModel);iOS;\(systemVersion); Wifi; \(appVersion)", dataArray: dataArray)
    ...
    func iTunesPushViewController(identifier : String, animated: Bool = true) {
        let storyBoard:UIStoryboard = UIStoryboard(name: "Main", bundle: nil)
        let viewController = storyBoard.instantiateViewController(withIdentifier: identifier)
        self.iTunesPushViewController(viewController: viewController, animated: animated)
    }
    
    func iTunesPresentViewController(viewController: UIViewController, animated: Bool = true, completion: (() -> Void)? = nil) {
        self.present(viewController, animated: animated, completion: completion)
    }
    
    func iTunesPushViewController(viewController: UIViewController, animated: Bool = true) {
